...
chibook 是一款本地优先的 Flutter 阅读与听书 App，视觉与交互依据 `docs/erdu-mockup.html` 实现。本地导入书籍、阅读设置、划线与听书记录仍只保存在设备本地；用户也可以选择登录微信读书，同步本人书架并在线按章阅读。

## 已实现

- 启动页与游客模式
- 微信读书扫码登录、安全凭证存储、书架同步与退出登录
- 微信读书书籍目录、云端阅读进度恢复、按需章节加载、正文阅读与 TTS 听书
- 阅读首页：继续阅读、真实本机统计、最近收听控制、最近阅读和上次划线
- 本地书架、网格/列表视图、置顶、删除、阅读进度
- 从系统文件选择器导入 TXT、EPUB、PDF
- 登录 Z-Library 后搜索并下载有权使用的 TXT、EPUB、PDF 到本地书架
...
登录与正文阅读使用微信读书网页端兼容接口。扫码成功后的 Cookie 只写入 `flutter_secure_storage`，不会进入 SharedPreferences 或仓库；远程书架只持久化书籍和章节元数据，章节正文按需加载且不落入书架 JSON。本地书籍与微信读书书籍使用独立来源标记，退出微信读书时不会影响本地导入内容。微信读书文字章节可使用设置页选择的系统、OpenAI 或 Azure TTS，连续播放切章时会自动加载下一章。

微信读书目前没有面向普通第三方阅读器的稳定公开正文 API，因此该能力可能受网页接口调整、账号会员状态、单本书购买权限和版权限制影响。当前仅支持普通电子书的文字正文；公众号合集、有声专辑和章节图片不会伪装成已完整支持。

### Z-Library

在“我的 → Z-Library”登录后，可从书架搜索页切换到 Z-Library，搜索并下载 TXT、EPUB 或 PDF。密码只用于当次登录，不会落盘；会话令牌保存在 `flutter_secure_storage`。下载文件先进入临时目录，解析成功后再复制到应用私有 `library` 目录，失败会清理临时文件。请仅下载公版、开放授权或已合法取得使用权的内容。该功能依赖非公开兼容接口，服务端域名、反自动化策略或下载配额变化可能造成暂时不可用。
...
PDF 使用开源 `pdfrx`/PDFium，与 TXT/EPUB 阅读器完全分离。PDF 是固定排版格式，只提供整体缩放和翻页，不伪装支持字号或行距重排。数据模型预留“页码 + 归一化矩形”标注定位。导入时优先逐页提取文本；无文本页、以及文本层乱码的页（中文 PDF 常见的字体缺 ToUnicode CMap，提取出的是 NUL 或私用区码点）在 Android/iOS 上使用端侧 Google ML Kit 中文模型 OCR 重新识别，OCR 也救不回来时该页记为无文本，不把乱码当正文存下来。分栏、页眉页脚、图像清晰度和字符映射仍可能影响朗读顺序与识别质量。
