...
登录与正文阅读使用微信读书网页端兼容接口。扫码成功后的 Cookie 只写入 `flutter_secure_storage`，不会进入 SharedPreferences 或仓库；远程书架只持久化书籍和章节元数据，章节正文按需加载且不落入书架 JSON。本地书籍与微信读书书籍使用独立来源标记，退出微信读书时不会影响本地导入内容。
