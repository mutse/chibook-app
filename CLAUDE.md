...
- **builtin** — `flutter_tts`, speaks from `_characterOffset` in sentence-aware chunks of at most 3000 UTF-16 code units so Android stays below `TextToSpeech.getMaxSpeechInputLength()`; `setProgressHandler` offsets are relative to the active chunk, so they're rebased onto `_speechStartOffset`.
