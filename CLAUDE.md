...
- **builtin** — `flutter_tts`, speaks from `_characterOffset`; Android uses one real sentence per utterance (capped at 3000 UTF-16 code units) so it stays below `TextToSpeech.getMaxSpeechInputLength()` and can use utterance start/completion as a truthful sentence-level fallback when a vendor engine omits range callbacks. When `setProgressHandler` is available its offsets are relative to the active chunk, so they're rebased onto `_speechStartOffset`.
